...
export const useShortcut = () => {
  ...
  const version = useShortcutStore((state) => state.version) // 监听版本变化
  ...
  const cleanupShortcuts = useCallback(async () => {
    logger.log('🔧 [快捷键系统] 开始清理快捷键...')
    try {
      await unregisterAll()
      logger.log('✅ [快捷键系统] 快捷键清理成功')
    } catch (error) {
      logger.error('❌ [快捷键系统] 快捷键清理失败:', error)
    }
  ...
  const registerShortcuts = useCallback(async () => {
    logger.log('🔧 [注册函数] 开始注册，当前 managerRef.current:', managerRef.current)

    // 防止重复初始化
    if (isInitializing.current) {
      logger.log('🔧 [快捷键系统] 正在初始化，跳过重复调用')
      return
    }
    ...
    // 如果快捷键配置没有变化，则跳过（version 变化会强制重新注册）
    if (
      lastRegisteredShortcuts.current === currentShortcutsString &&
      registrationId.current !== null
    ) {
      logger.log('🔧 [快捷键系统] 快捷键配置未变化，跳过重新注册')
      return
    }

    isInitializing.current = true
    logger.log('🔧 [快捷键系统] 开始注册全局快捷键...', shortcuts)
    ...
    try {
      ...
      if ((window as any)[registrationKey]) {
        logger.log(`🔧 [注册去重] 快捷键 ${toggleAccelerator} 已经注册过，跳过`)
        return
      }
      ...
      await register(toggleAccelerator, async () => {
        ...
        logger.log(`🔧 [调试] 进入快捷键回调函数，触发ID:`, triggerId)
        ...
        if (isCurrentlyPressed) {
          ...
          logger.log(`🚀 [快捷键触发] 检测到松开事件，忽略: ${toggleAccelerator}`)
        ...
        logger.log(`🚀 [快捷键触发] 检测到按下事件: ${toggleAccelerator}`)
        logger.log('🚀 [快捷键触发] 准备调用 toggleWindow...')

        try {
          await useAppStore.getState().toggleWindow()
          logger.log('🚀 [快捷键触发] toggleWindow 调用完成')
        } catch (error) {
          logger.error('❌ [快捷键触发] toggleWindow 调用失败:', error)
        }
      })

      logger.log(`🔧 [快捷键系统] 注册设置快捷键: ${settingsAccelerator}`)
      await register(settingsAccelerator, async () => {
        ...
        if (isCurrentlyPressed) {
          // 当前是按下状态，这次触发是松开事件，忽略
          logger.log(`🚀 [快捷键触发] 检测到松开事件，忽略: ${settingsAccelerator}`)
        ...
        logger.log(`🔧 [快捷键触发] 检测到按下事件: ${settingsAccelerator}`)
        logger.log('🔧 [快捷键触发] 准备调用 toggleSettings...')

        try {
          await useAppStore.getState().toggleSettings()
          logger.log('🚀 [快捷键触发] toggleSettings 调用完成')
        } catch (error) {
          logger.error('❌ [快捷键触发] toggleSettings 调用失败:', error)
        }
      })

      logger.log('✅ [快捷键系统] 所有快捷键注册成功:', {
      ...
      logger.log('✅ [注册函数] 成功设置全局单例:', managerRef.current)
      ...
      logger.error('❌ [快捷键系统] 快捷键注册失败:', error)
      await cleanupShortcuts()
    } finally {
      isInitializing.current = false
    }
  ...
  useEffect(() => {
    ...
    logger.log('🔧 [useEffect] useEffect 被调用，依赖:', {
    ...
    const initializeShortcuts = () => {
      ...
      timeoutId = window.setTimeout(() => {
        logger.log('🔧 [useEffect] 执行 registerShortcuts 调用')
        registerShortcuts()
      }, 100)
    }

    logger.log('🔧 [useEffect] 准备调用 initializeShortcuts')
    initializeShortcuts()

    return () => {
      logger.log('🔧 [useEffect] 清理 useEffect（不清理全局快捷键，避免 hot reload 失效）')
      if (timeoutId) {
        window.clearTimeout(timeoutId)
      }
      // 不调用 cleanupShortcuts()，因为 hot reload 时会导致快捷键被注销
      // 快捷键的清理会在重新注册前自动进行
    }
  }, [registerShortcuts, cleanupShortcuts, version]) // 添加 version 依赖
}
